...
int main(int argc, char *argv[])
{
    ...
    if (!is_number(x_str))
    {
        ...
        return 2; // Error: Invalid number format
    }
...
int handle_s_flag(const char *x_str)
{
    ...
    char hex_representation[32]; // Assuming max 32-bit integer
    int index = 0;
    ...
    // Print in reverse order (from most significant to least significant digit)
...
int handle_e_flag(const char *x_str)
{
    ...
    if (x > MAX_X_FOR_E)
    {
        ...
        return 4; // Error: x exceeds limit for -e flag
    }
...
int handle_f_flag(const char *x_str)
{
    ...
    if (x < 0)
    {
        ...
        return 5; // Error: Negative number for factorial
    }
    ...
    else
    {
        ...
        for (int i = 2; i <= n; i++)
        {
            if ((LLONG_MAX / result) < i)
            {
                ...
                exit(6); // Error: Factorial overflow (exit allowed here due to overflow)
            }
            result *= i;
        }
        return result;
